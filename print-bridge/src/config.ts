...
if (fs.existsSync(execConfigPath) && isWritable(execConfigPath)) {
  ...
  packageConfigPath &&
  fs.existsSync(packageConfigPath) &&
  isWritable(packageConfigPath)
) {
  ...
  if (!fs.existsSync(userConfigPath)) {
    ...
    if (fs.existsSync(execConfigPath)) templateSource = execConfigPath;
      ...
      templateSource = packageConfigPath;
...
const defaultConfig: BridgeConfig = {
  ...
  backends: [
    ...
    {
      name: "QR POS",
      url: "https://qr-kindee-production.up.railway.app",
      enabled: true,
    },
  ],
};
...
function loadConfig(): BridgeConfig {
  if (!fs.existsSync(finalConfigPath)) {
    logger.warn(
      `Could not find config.json in any path. Using built-in defaults.`,
    );
  ...
  try {
    ...
    if (!parsed.backends || !Array.isArray(parsed.backends)) {
      ...
      logger.info(
        `Conversions: Mapped legacy URL configuration into multi-backends.`,
      );
    }
    ...
    logger.error(
      `Error reading config.json: ${err.message}. Using built-in defaults.`,
    );
    return defaultConfig;
  }
}
